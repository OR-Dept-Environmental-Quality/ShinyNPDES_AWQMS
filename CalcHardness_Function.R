...
hardness<-function(x){
  ...
  #note that umho/cm and uS/cm don't need any conversion.
  cond<-subset(x,x$Char_Name %in% c("Conductance","Specific conductance","Specific conductivity","Conductivity") & x$Result_Unit %in% c("umho/cm","uS/cm"))
  ...
  #remove activity id's with no Ca, Mg, or conductance data that made it through
